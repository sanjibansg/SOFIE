...
public:
   ...
   std::string Generate_GPU_Kernel_ALPAKA(std::string /*opName*/) override {
      ...
      op += SP + SP + SP + "for (std::size_t j = 0; j < " + std::to_string(fInputLength) + "u; ++j) n += input[j] != static_cast<T>(0);\n";
      op += SP + SP + SP + "*count = n;\n";
   ...
   std::string Generate_GPU_ALPAKA(std::string /*opName*/) override {
      ...
      out << SP << "auto taskNonZero_" << fNY << " = alpaka::createTaskKernel<Acc>(workDivNonZero_" << fNY << ", nonZeroKernel_" << fNY << ", alpaka::getPtrNative(deviceBuf_" << fNX << "), alpaka::getPtrNative(bufDev_" << fNY << "), alpaka::getPtrNative(nonZeroCountDev_" << fNY << "));\n";
