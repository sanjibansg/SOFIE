...
std::string Generate_GPU_Kernel_ALPAKA(std::string opName) override {
   ...
   const std::size_t axis = static_cast<std::size_t>(fAxis);
   const auto outStrides = UTILITY::ComputeStrideFromShape(fOutputShape);
   const std::size_t innerSize = std::stoul(outStrides[axis].GetVal());

   std::vector<std::size_t> inputBlockSizes(Nin);
   std::vector<std::size_t> prefixElements(Nin);
   std::size_t outputBlockSize = 0;

   for (std::size_t k = 0; k < Nin; ++k) {
      prefixElements[k] = outputBlockSize;
      inputBlockSizes[k] = std::stoul(fInputShapes[k][axis].GetVal()) * innerSize;
      outputBlockSize += inputBlockSizes[k];
   }
   ...
   op += SP + SP + SP + SP + "std::size_t const outer_idx = elem_idx / " + std::to_string(outputBlockSize) + "u;\n";
   op += SP + SP + SP + SP + "std::size_t const within_outer = elem_idx - outer_idx * " + std::to_string(outputBlockSize) + "u;\n\n";

   if (Nin == 1) {
      op += SP + SP + SP + SP + "output[elem_idx] = inputs[0][outer_idx * " + std::to_string(inputBlockSizes[0]) + "u + within_outer];\n";
   } else {
      for (std::size_t k = 0; k < Nin; ++k) {
         if (k == 0) {
            op += SP + SP + SP + SP + "if (within_outer < " + std::to_string(prefixElements[k] + inputBlockSizes[k]) + "u) {\n";
         } else if (k + 1 < Nin) {
            op += SP + SP + SP + SP + "else if (within_outer < " + std::to_string(prefixElements[k] + inputBlockSizes[k]) + "u) {\n";
         } else {
            op += SP + SP + SP + SP + "else {\n";
         }

         op += SP + SP + SP + SP + SP + "output[elem_idx] = inputs[" + std::to_string(k) + "][outer_idx * " + std::to_string(inputBlockSizes[k]) + "u + (within_outer - " + std::to_string(prefixElements[k]) + "u)];\n";
         op += SP + SP + SP + SP + "}\n";
      }
   }
