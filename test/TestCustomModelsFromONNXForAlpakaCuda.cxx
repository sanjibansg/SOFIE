...
#include "AddBroadcast2_FromONNX_GPU_ALPAKA.hxx"
#include "input_models/references/AddBroadcast2.ref.hxx"

#include "AddBroadcast3_FromONNX_GPU_ALPAKA.hxx"
#include "input_models/references/AddBroadcast3.ref.hxx"

#include "AddBroadcast4_FromONNX_GPU_ALPAKA.hxx"
#include "input_models/references/AddBroadcast4.ref.hxx"

#include "AddBroadcast5_FromONNX_GPU_ALPAKA.hxx"
#include "input_models/references/AddBroadcast5.ref.hxx"

#include "AddBroadcast6_FromONNX_GPU_ALPAKA.hxx"
#include "input_models/references/AddBroadcast6.ref.hxx"

#include "AddBroadcast7_FromONNX_GPU_ALPAKA.hxx"
...
TEST_F(SofieAlpakaTest, AddBroadcast2)
{
   constexpr float TOLERANCE = DEFAULT_TOLERANCE;

   std::vector<float> A_vec({
      0.60081805, 0.56575772, -0.58408511, -1.50827751, 1.2396254
   });
   ...
   auto A_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{A_vec.size()}));
   auto B_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{B_vec.size()}));
   ...
   auto A_d = alpaka::allocBuf<float, Idx>(device, Ext1D::all(Idx{A_vec.size()}));
   auto B_d = alpaka::allocBuf<float, Idx>(device, Ext1D::all(Idx{B_vec.size()}));

   alpaka::memcpy(queue, A_d, A_h);
   alpaka::memcpy(queue, B_d, B_h);
   alpaka::wait(queue);

   constexpr size_t outputSize = sizeof(AddBroadcast2_ExpectedOutput::output) / sizeof(float);
   auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

   {
      SOFIE_AddBroadcast2::Session<alpaka::TagGpuCudaRt> session;
      auto result = session.infer(A_d, B_d);
      alpaka::wait(queue);
      cudaDeviceSynchronize();
   ...
   float *result = reinterpret_cast<float *>(alpaka::getPtrNative(result_h));
   float *correct = AddBroadcast2_ExpectedOutput::output;

   for (size_t i = 0; i < outputSize; ++i)
      EXPECT_LE(std::abs(result[i] - correct[i]), TOLERANCE) << "i=" << i;
}

TEST_F(SofieAlpakaTest, AddBroadcast3)
{
   constexpr float TOLERANCE = DEFAULT_TOLERANCE;

   std::vector<float> A_vec({
      0.13225244, -0.47801406, -1.47034622, 0.87786363, -0.51388502,
   ...
   constexpr size_t outputSize = sizeof(AddBroadcast3_ExpectedOutput::output) / sizeof(float);
   auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

   {
      SOFIE_AddBroadcast3::Session<alpaka::TagGpuCudaRt> session;
      auto result = session.infer(A_d, B_d);
      alpaka::wait(queue);
      cudaDeviceSynchronize();
...
TEST_F(SofieAlpakaTest, AddBroadcast4)
{
    constexpr float TOLERANCE = DEFAULT_TOLERANCE;

    std::vector<float> A_vec({
       1.94301397, 0.40606817
    });

    std::vector<float> B_vec({
       0.50898894, -0.27829921, -0.68761628, 0.33186382,
       0.57915535, 0.406858, 1.4203833, 0.19857093
    });
    ...
    constexpr size_t outputSize = sizeof(AddBroadcast4_ExpectedOutput::output) / sizeof(float);
    auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

    {
        SOFIE_AddBroadcast4::Session<alpaka::TagGpuCudaRt> session;
        auto result = session.infer(A_d, B_d);
        alpaka::wait(queue);
        cudaDeviceSynchronize();
...
TEST_F(SofieAlpakaTest, AddBroadcast5)
{
   constexpr float TOLERANCE = DEFAULT_TOLERANCE;

   std::vector<float> A_vec({
      -0.45616139, -0.05853134, 1.09564217, 0.95880315,
      0.94995322, -0.35864105, 1.08570897, 0.6028053
   });

   std::vector<float> B_vec({
      1.69787452, 1.10641673, 2.19755165, 0.06709206,
      0.04572308, -2.14504366, -0.47730702, 0.15205423,
      -0.25159224, -0.07529807, 0.5174367, 0.08267595,
      0.34015625, 0.09460231, -1.16608969, -0.23466058,
      -0.5520268, -0.13844847, 0.53055759, 0.17068648,
   ...
   constexpr size_t outputSize = sizeof(AddBroadcast5_ExpectedOutput::output) / sizeof(float);
   auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

   {
      SOFIE_AddBroadcast5::Session<alpaka::TagGpuCudaRt> session;
      auto result = session.infer(A_d, B_d);
      alpaka::wait(queue);
      cudaDeviceSynchronize();
...
TEST_F(SofieAlpakaTest, AddBroadcast6)
{
   constexpr float TOLERANCE = DEFAULT_TOLERANCE;

   std::vector<float> A_vec({
      1.05498675, -1.64311041, 0.11925147, -1.59755778,
      -0.01445313, -0.69440541, -0.12011281, 0.00539323,
      -0.16923531, 2.34533598, 1.30268048, 0.45699443
   });

   std::vector<float> B_vec({
      0.03162163, 1.36340443, -0.34736459, -0.71856324,
      0.40669968, -0.37595741, 0.22234952, 1.69563792,
      0.91459166, -0.02081215, -1.64894217, -0.01189261,
      0.58031339, -0.11880191, 0.70099317, -0.37424243,
      -0.23980527, -0.03178407, -0.27969109, 0.01895688,
      1.32111755, 0.02113906, 0.51450298, -1.41760768,
      -0.19220553, 0.23529522, 0.95199908, -1.38971445,
      -0.75836965, -0.90956958, -0.13006828, -0.64390454,
      -0.0808229, 0.79134757, 1.00684867, -1.43818087,
      -0.14550621, -0.33635512, -0.6185612, -0.49281407,
      -1.12947258, 1.61818821, -0.05826431, -1.47802183,
      0.25637381, -0.1547858, 2.50788792, 0.30898059
   });
   ...
   constexpr size_t outputSize = sizeof(AddBroadcast6_ExpectedOutput::output) / sizeof(float);
   auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

   {
      SOFIE_AddBroadcast6::Session<alpaka::TagGpuCudaRt> session;
      auto result = session.infer(A_d, B_d);
      alpaka::wait(queue);
      cudaDeviceSynchronize();
...
TEST_F(SofieAlpakaTest, AddBroadcast7)
{
    constexpr float TOLERANCE = DEFAULT_TOLERANCE;

    std::vector<float> A_vec({
       -0.42164834, -0.61767078, -0.68778897,
       -1.14175916, 0.63204375, -0.60630317
    });

    std::vector<float> B_vec({
       1.40519865e+00, -2.87660856e-01, 7.49375999e-02, 1.22074840e+00,
    ...
    constexpr size_t outputSize = sizeof(AddBroadcast7_ExpectedOutput::output) / sizeof(float);
    auto result_h = alpaka::allocBuf<float, Idx>(host, Ext1D::all(Idx{outputSize}));

    {
        SOFIE_AddBroadcast7::Session<alpaka::TagGpuCudaRt> session;
        auto result = session.infer(A_d, B_d);
        alpaka::wait(queue);
        cudaDeviceSynchronize();
